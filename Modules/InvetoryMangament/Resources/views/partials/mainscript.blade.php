    ...
    $('#search_product_inventory')
        .autocomplete({
            source: function (request, response) {
                $.getJSON(
                    '/invetorymangament/inventory/get_products/'+last_part,
                    {term: request.term},
                    response
                );
            },
            ...
            response: function (event, ui) {
                ...
                if(ui.content[0]?.NotFound ==  true){
                    ...
                    return toastr.error("@lang('invetorymangament::inventory.product_notfound')");
//  ("");
                }
...
function get_purchase_entry_row(product_id, variation_id , last_part) {
    ...
    if (product_id) {
        ...
        $.ajax({
            method: 'POST',
            url: '/invetorymangament/inventory/get_purchase_entry_row',
            dataType: 'html',
            data: data,
...
if(amountAfterInventoryFlag){
    toastr.error("@lang('invetorymangament::inventory.check_qty_added_msg')");
}
else{
...
$.ajax({
    type: 'POST',
    url: '/invetorymangament/saveInventoryProducts',
    dataType: "json",
    data: {info: obj},
    success: function(result) {
        if(result.status == 200){
          window.location.href = '/invetorymangament/showInventoryList';
        }
    },
...
$(document).on('click','.edit_current_row',function (e){
    ...
    $.ajax({
    type: 'GET',
    url: '/invetorymangament/getProductData',
    data: {values: values},
    success: function(result) {
...
$(document).on('click','#save_new_inv_qty',function (e){
    ...
    if($qty !=  qty_inv){
        swal({
            title: `"@lang('invetorymangament::inventory.alert_modal_msg')" : (${$qty})`,
            icon: 'warning',
            buttons: true,
            dangerMode: true,
        }).then(willDelete => {
            if (willDelete) {
                $btn.attr('disabled','true');
                $.ajax({
                    url:'/invetorymangament/updateProductQuantity',
                    method:'POST',
