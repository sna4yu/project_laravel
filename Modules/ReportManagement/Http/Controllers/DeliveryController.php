...
class DeliveryController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            $datatable = Datatables::of($sells)
                    ...
                    function ($row) use ($only_shipments, $is_admin, $sale_type) {
                        ...
                        if ($row->type == 'sell') {
                            ...
                            if ($is_admin || auth()->user()->hasAnyPermission(['access_shipping', 'access_own_shipping', 'access_commission_agent_shipping'])) {
                                $html .= '<li><a href="#" data-href="' . action([\App\Http\Controllers\SellController::class, 'editShipping'], [$row->id]) . '" class="btn-modal" data-container=".view_modal"><i class="text-danger fas fa-truck" aria-hidden="true"></i>' . __('lang_v1.edit_shipping') . '</a></li>';
                            }
                        ...
                        $delete_link = '<li><a href="' . action([\App\Http\Controllers\SellPosController::class, 'destroy'], [$row->id]) . '" class="delete-sale"><i class="text-danger fas fa-trash"></i> ' . __('messages.delete') . '</a></li>';
                        if ($row->is_direct_sale == 0) {
                ...
                ->editColumn('action_status', function ($row) use ($is_admin) {
                    ...
                    $label = !empty($row->audit_status) ? $row->audit_status : 'Pending';
                    $status_color = $action_status[$label] ?? $action_status['Default']; // Use default color if label is not found
    
                    // $editable = $is_admin || auth()->user()->hasAnyPermission(['audit_status'] );
                    $editable = $is_admin || auth()->user()->can('audit_status');//it the same
    
                    $action_url = action([\App\Http\Controllers\SellController::class, 'editActionStatus'], [$row->id]);

                    // Conditional rendering based on whether the user can edit
                    if ($editable) {
                        return '<a href="#" class="btn-modal" data-href="' . $action_url . '" data-container=".view_modal"><span class="label ' . $status_color . '">' . $label . '</span></a>';
                    } else {
                        return '<span class="label ' . $status_color . '">' . $label . '</span>';
                    }
                })
                ...
                ->editColumn('name', function ($row) {
                    ...
                    return '<a href="' . action([\App\Http\Controllers\ContactController::class, 'show'], [$row->contact_id]) . '" target="_blank" class="no-print">' .
                        $name .
                        '</a>';
                })
            ...
            $rawColumns = ['name', 'action_status', 'final_total', 'action', 'total_paid', 'total_remaining', 'payment_status', 'invoice_no', 'discount_amount', 'tax_amount', 'total_before_tax', 'shipping_status', 'types_of_service_name', 'payment_methods', 'return_due', 'contact_name', 'status'];
        ...
        return view('reportmanagement::Delivery_report.shipments')
    ...
    {
        // Get filter parameters with default values if they are not provided
        ...
        $counts = $query->selectRaw('shipping_status, COUNT(*) as count')
