...
class AttendanceController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($attendance)
                ->addColumn(
                    'action',
                    '@can("essentials.crud_all_attendance") <button data-href="{{action(\'\Modules\Essentials\Http\Controllers\AttendanceController@edit\', [$id])}}" class="btn btn-xs btn-navy btn-modal" data-container="#edit_attendance_modal"><i class="glyphicon glyphicon-edit"></i> @lang("messages.edit")</button>
    ...
    {
        $employees = User::pluck('name', 'id')->prepend('Select Employee', '');
   ...
   public function exportattendance(Request $request)
{
    ...
    $dateRange = explode(' - ', $validatedData['date_range']);
    if (count($dateRange) !== 2) {
        return back()->withErrors(['date_range' => 'The date range format is invalid. It must be "DD-MM-YYYY - DD-MM-YYYY" or "MM/DD/YYYY - MM/DD/YYYY".']);
    }

    // Determine the separator used in the date format
    $separator = strpos($dateRange[0], '/') !== false ? '/' : '-';
    $format = $separator === '/' ? 'm/d/Y' : 'd-m-Y';

    try {
        $startDate = Carbon::createFromFormat($format, trim($dateRange[0]))->startOfDay();
        $endDate = Carbon::createFromFormat($format, trim($dateRange[1]))->endOfDay();
    } catch (\Exception $e) {
        return back()->withErrors(['date_range' => 'Invalid date format. Please use the correct format for your dates.']);
    }

    if ($endDate->lessThan($startDate)) {
        return back()->withErrors(['date_range' => 'Invalid date range. The end date must be after the start date.']);
    }

    // Retrieve business_id from the session (uncomment and adjust if needed)
    $business_id = $request->session()->get('user.business_id');

    // Pass the business_id to the AttendanceExport (adjust constructor in AttendanceExport if needed)
    $export = new AttendanceExport($startDate->toDateString(), $endDate->toDateString(), $business_id);
    $fileName = 'attendance-' . $startDate->format('Y-m-d') . '-to-' . $endDate->format('Y-m-d') . '.xlsx';
    ...
    public function getAttendanceRow($user_id)
    {
        ...
        return view('attendance_report')->with(compact('attendance', 'shifts', 'user'));
    }
}
