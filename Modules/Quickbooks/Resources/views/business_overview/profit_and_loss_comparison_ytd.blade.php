...
@section('title', __('Profit and Loss YTD Comparison Report'))
    ...
    <section class="content" style="background-color: #ffffff;">
        <div class="row" style="padding: 20px; background-color: #ffffff;">
            <div class="box-body" style="background-color: #F4F5F8;">
                ...
                <!-- Title -->
                <h3 style="font-size: 18px; font-weight: bold; margin-bottom: 20px; padding-left: 12px;">Profit and Loss YTD
        ...
        <div class="col-md-10 col-md-offset-1" style="background-color: #ffffff;">
            <div class="box" style="background-color: #ffffff;">
                <div class="box-header with-border text-center">
                    ...
                    <h5 class="box-title"><b>Profit and Loss YTD Comparison Report</b></h5>
                    <p>{{ strtoupper(\Carbon\Carbon::parse(request('start_date', \Carbon\Carbon::now()))->format('j M, Y')) }}
                        -
                        {{ strtoupper(\Carbon\Carbon::parse(request('end_date', \Carbon\Carbon::now()))->format('j M, Y')) }}
                    </p>
                </div>
                    ...
                    <table class="table table-striped table-bordered"
                        ...
                        <thead>
                            <tr>
                                ...
                                <th
                                    style="width: 60%; border-top: 1px solid #000; border-right: 1px solid #000;  text-align:left;">
                                </th>
                            ...
                            <tr>
                                <!-- Empty header for the first column -->
                                <th style="border-right: 1px solid #000; border-bottom: 1px solid #000; text-align:left;">
                                </th>

                                <!-- Current period (start_date - end_date) -->
                                <th
                                    style="border-right: 1px solid #000; border-left: 1px solid #000; border-bottom: 1px solid #000; text-align:right;">
                                    {{ strtoupper(\Carbon\Carbon::parse(request('start_date', \Carbon\Carbon::now()))->format('j M, Y')) }}
                                    -
                                    {{ strtoupper(\Carbon\Carbon::parse(request('end_date', \Carbon\Carbon::now()))->format('j M, Y')) }}
                                </th>

                                <!-- Year-to-Date (YTD) period (from 1st Jan to end_date) -->
                                <th style="border-left: 1px solid #000; border-bottom: 1px solid #000; text-align:right;">
                                    1 JAN
                                    {{ strtoupper(\Carbon\Carbon::parse(request('end_date', \Carbon\Carbon::now()))->format('Y')) }}
                                    -
                                    {{ strtoupper(\Carbon\Carbon::parse(request('end_date', \Carbon\Carbon::now()))->format('j M, Y')) }}
                                    (YTD)
                                </th>
                            ...
                            @foreach ($filtered_account_types as $type => $details)
                                @php
                                    $primary_type_total_balance = $accounts
                                        ...
                                        ->sum('ytd_balance'); // Sum YTD balance
                                @endphp
                                                        ...
                                                        <table class="table" style="display: none;">
                                                            @php
                                                                $subtype_accounts = $accounts
                                                                    ...
                                                                    ->sortBy('name');
                                                                $total_balance = $subtype_accounts->sum('ytd_balance'); // YTD balance for sub-types
                                                            @endphp
                                                            @foreach ($subtype_accounts as $account)
                                                                <tr class="account-row"
                                                                    data-balance="{{ $account->ytd_balance }}"
                                                                    ...
                                                                    <td style="text-align: right; width: 20%">
                                                                        @format_currency($account->ytd_balance)
                                                                    </td>
                                                                </tr>
                                                            @endforeach
                                                            <tr
                                                                style="border-top: 1px solid #000; font-weight: bold; background-color: #ffffff;">
                                                                <td style="width: 60%;">Total for
                                                                    {{ $sub_type->account_type_name }}</td>
                                                                <td style="text-align: right; width:20%;">@format_currency($total_balance)
                                                                </td>
                                                            </tr>
                                                        </table>
                                                    </td>
        ...
        $(document).ready(function() {
            ...
            $('button').each(function() {
                $(this).html($(this).html().replace('&#9654;',
                    '&#9660;')); // Set all arrows down by default
            });
        });
    </script>
@endsection
