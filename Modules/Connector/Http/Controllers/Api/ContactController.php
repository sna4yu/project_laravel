...
class ContactController extends ApiController
{
    ...
    public function index()
    {
        ...
        // $has_permission = $is_admin || $user->hasAnyPermission($permissions, 'web');
        ...
        $query = Contact::where('contacts.business_id', $business_id)
            ...
            ->leftJoin('users AS u', 'uca.user_id', '=', 'u.id')
            ->leftJoin('transactions AS t', 'contacts.id', '=', 't.contact_id')
            ->leftJoin('customer_groups AS cg', 'contacts.customer_group_id', '=', 'cg.id')
            ->select(
                'contacts.*',
                'cg.name as customer_group',
                'uca.user_id AS assigned_to_user_id',
                DB::raw("CONCAT(u.first_name, ' ', COALESCE(u.last_name, '')) AS assigned_to_user_name"),
                DB::raw("SUM(IF(t.type = 'opening_balance', t.final_total, 0)) as opening_balance"),
                DB::raw("SUM(IF(t.type = 'sell', t.final_total, 0)) as total_invoice"),
                ...
                DB::raw('contacts.balance as contact_balance')
            )
            ->groupBy('contacts.id');

        // Apply type filters
        if ($type == 'supplier' && $user->hasPermissionTo('supplier.view', 'web')) {
            $query->where('contacts.type', 'supplier');
        } elseif ($type == 'customer' && $user->hasPermissionTo('customer.view', 'web')) {
            $query->where('contacts.type', 'customer');
        } else {
            if ($user->hasPermissionTo('customer.view_own', 'web') || $user->hasPermissionTo('supplier.view_own', 'web')) {
                $query->where('contacts.created_by', $user->id);
            }
        }

        // Add the filters from indexCustomer
        ...
        if (request()->has('has_advance_balance')) {
            $query->havingRaw('contact_balance > 0');
        }
        ...
        if (!empty(request()->input('assigned_to'))) {
            $query->where('uca.user_id', request()->input('assigned_to'));
        }
        ...
        // Filter by no sales from specific periods
        $has_no_sell_from = request()->input('has_no_sell_from', null);
        $this->applyNoSellFilter($query, $has_no_sell_from, $is_admin);

        // Apply other filters (search, ordering)
        $search = request()->only(['name', 'biz_name', 'mobile_num', 'contact_id', 'customer_group_id']);
        if (!empty($search)) {
            $query->where(function ($query) use ($search) {
                ...
                if (!empty($search['customer_group_id'])) {
                    $query->where('contacts.customer_group_id', $search['customer_group_id']);
                }
            });
        }
    ...
    // Helper method to filter contacts with no sales from specific periods
    private function applyNoSellFilter($query, $has_no_sell_from, $is_admin)
    {
        $user = Auth::user();

        // Define the date range based on the 'has_no_sell_from' input
        if ($has_no_sell_from) {
            // Set the date threshold based on the input period
            $from_transaction_date = null;
            switch ($has_no_sell_from) {
                case 'one_month':
                    if ($is_admin || $user->can('customer_with_no_sell_one_month') || $user->can('customer_irrespective_of_sell')) {
                        $from_transaction_date = \Carbon::now()->subDays(30)->format('Y-m-d');
                    }
                    break;
                case 'three_months':
                    if ($is_admin || $user->can('customer_with_no_sell_three_month') || $user->can('customer_irrespective_of_sell')) {
                        $from_transaction_date = \Carbon::now()->subMonths(3)->format('Y-m-d');
                    }
                    break;
                case 'six_months':
                    if ($is_admin || $user->can('customer_with_no_sell_six_month') || $user->can('customer_irrespective_of_sell')) {
                        $from_transaction_date = \Carbon::now()->subMonths(6)->format('Y-m-d');
                    }
                    break;
                case 'one_year':
                    if ($is_admin || $user->can('customer_with_no_sell_one_year') || $user->can('customer_irrespective_of_sell')) {
            ...
            if ($from_transaction_date) {
                // Filter contacts that have no sales after the specified date
                $query->havingRaw("(MAX(t.transaction_date) < '{$from_transaction_date}' OR MAX(t.transaction_date) IS NULL)");
            }
        }
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $input['opening_balance'] = $this->num_uf($request->input('opening_balance'));
    ...
    public function show($contact_ids)
    {
        ...
        ->leftJoin('users AS u', 'uca.user_id', '=', 'u.id')
        ->addSelect(
            DB::raw("GROUP_CONCAT(DISTINCT uca.user_id) AS assigned_to_user_ids"),
            DB::raw("GROUP_CONCAT(DISTINCT CONCAT(u.first_name, ' ', COALESCE(u.last_name, ''))) AS assigned_to_user_names")
        )
        ->groupBy('contacts.id')
        ->get();
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $input = $request->only([
                ...
                'opening_balance',
                'name',
                'position' // Added 'name' explicitly
            ]);

            // Convert the opening balance
            $input['opening_balance'] = $this->num_uf($request->input('opening_balance'));
            ...
            if ($input['type'] == 'lead') {
                ...
                $output = $this->contactUtil->updateContact($input, $id, $business_id); // Directly passing $input for update
            }
    ...
    public function num_uf($input_number, $currency_details = null)
    {
        ...
        // Check if currency details are provided in the request
