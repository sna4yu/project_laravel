...
class SellController extends ApiController
{
    ...
    public function index()
    {
        ...
        // Abort if the user does not have the required permissions
        if (!$has_permission) {
            abort(403, 'Unauthorized action.');
        }

        $filters = request()->only([
            'location_id', 'contact_id', 'payment_status', 'start_date', 'end_date', 'user_id', 'service_staff_id', 'only_subscriptions', 'per_page', 'shipping_status', 'order_by_date', 'source', 'status',
            'customer_name', 'invoice_no', 'mobile'
        ]);
        ...
        if (!$user->hasPermissionTo('direct_sell.view', 'web')) {
            $query->where(function ($q) use ($user) {
                if ($user->hasPermissionTo('view_own_sell_only', 'web') || $user->hasPermissionTo('access_own_shipping', 'web')) {
                    $q->where('transactions.created_by', $user->id);
                }

                //if user is commission agent display only assigned sells
                if ($user->hasPermissionTo('view_commission_agent_sell', 'web') || $user->hasPermissionTo('access_commission_agent_shipping', 'web')) {
        ...
        if (!$is_admin) {
            $payment_status_arr = [];
            if (auth()->user()->hasPermissionTo('view_paid_sells_only', 'web')) {
                $payment_status_arr[] = 'paid';
            }

            if (auth()->user()->hasPermissionTo('view_due_sells_only', 'web')) {
                $payment_status_arr[] = 'due';
            }

            if (auth()->user()->hasPermissionTo('view_partial_sells_only', 'web')) {
                $payment_status_arr[] = 'partial';
            }

            if (empty($payment_status_arr)) {
                if (auth()->user()->hasPermissionTo('view_overdue_sells_only', 'web')) {
                    $query->OverDue();
                }
            } else {
                if (auth()->user()->hasPermissionTo('view_overdue_sells_only', 'web')) {
        ...
        if (!empty($filters['invoice_no'])) {
            $query->where('transactions.invoice_no', 'like', '%' . $filters['invoice_no'] . '%');
        }

        if (!empty($filters['mobile'])) {
            $query->where('c.mobile', 'like', '%' . $filters['mobile'] . '%');
        }
        ...
        foreach ($transactions as $transaction) {
            $totals['total_final_amount'] += $transaction->final_total;
            $totals['total_invoices']++;

            switch ($transaction->payment_status) {
                case 'due':
                    $totals['total_due_invoices']++;
                    $totals['total_due_amount'] += $transaction->final_total;
                    break;
                case 'paid':
                    $totals['total_paid_invoices']++;
                    $totals['total_paid_amount'] += $transaction->final_total;
                    break;
                case 'partial':
                    $totals['total_partial_invoices']++;
                    $totals['total_partial_amount'] += $transaction->final_total;
                    break;
            }
        }
        ...
        return response()->json([
            ...
            'links' => [
                'first' => $sells->url(1),
                'last' => $sells->url($sells->lastPage()),
                'prev' => $sells->previousPageUrl(),
                'next' => $sells->nextPageUrl(),
            ],
            'meta' => [
                'current_page' => $sells->currentPage(),
                'from' => $sells->firstItem(),
                'last_page' => $sells->lastPage(),
                'links' => $sells->linkCollection()->toArray(),
                'path' => $sells->path(),
    ...
    //audit_status of the transaction
    public function updateAuditStatus(Request $request, $id)
    {
        try {
            ...
            $old_status = $transaction->audit_status;

            // Update transaction
            $transaction->audit_status = $request->input('audit_status');
            $transaction->save();
    ...
    public function show($sell_ids)
    {
        ...
        $with = ['sell_lines', 'payment_lines', 'sell_lines.product', 'media', 'activities', 'sales_person', 'delivery_person_user', 'sale_commission_agent', 'location'];
        ...
        try {
            ...
            foreach ($sells as $sell_data) {
                try {
                    ...
                    $change_return['is_return'] = 0;
                    $input['payment'][] = $change_return;

                    $doc = $this->transactionUtil->uploadFile($request, 'document', 'documents');
                    // Create or update payment lines
                    if (!empty($input['payment']) && $transaction->is_suspend == 0) {
                        $this->transactionUtil->createOrUpdatePaymentLines($transaction, $input['payment'], $doc, $business_id, $user->id, false);
                    }

                    // Finalize the transaction if the status is final
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            //Update Sell lines
            $input['payment'] = isset($input['payment']) ? $input['payment'] : null;
            ...
            if (!empty($input['payment']) && $transaction->is_suspend == 0) {
                $change_return = $this->dummyPaymentLine;
                $change_return['amount'] = $input['payment']['amount'];
                $change_return['is_return'] = 0;
    ...
    public function storeTransactionPayment(Request $request)
    {
        try {
            ...
            // Check if the transaction is not fully paid
            if ($transaction->payment_status != 'paid') {
                // Prepare the input data for the TransactionPayment
                $inputs = $request->only([
                    'amount', 
                    'method', 
                    'paid_on', 
                ...
                $inputs['payment_ref_no'] = $this->transactionUtil->generateReferenceNumber($prefix_type, $ref_count, $business_id);
    
                // Assign business ID and handle file upload if necessary
                ...
                // Create and save the transaction payment
                $tp = new TransactionPayment($inputs);
                $tp->save();
    
                // Update payment status of the transaction
                ...
                // Log the saved transaction payment
                \Log::info('TransactionPayment saved and payment status updated successfully', ['transaction_payment' => $tp]);
    
                // Return success response
                return response()->json([
                    'success' => true,
                    'msg' => 'Payment added successfully and payment status updated!',
                    'transaction_payment' => $tp
                ]);
            } else {
                // If the transaction is already fully paid, return a message
                return response()->json([
                    'success' => false,
                    'msg' => 'This transaction is already fully paid.',
                ]);
            }
        } catch (\Exception $e) {
            ...
            // Log any errors
            \Log::error('Error saving TransactionPayment or updating payment status:', ['error' => $e->getMessage()]);
            
            // Return error response
            return response()->json([
                'success' => false,
                'msg' => 'Failed to add payment or update payment status.'
            ], 500);
        }
    }
    ...
    public function updateSellShippingStatus(Request $request)
    {
        try {
            ...
            if (array_key_exists($shipping_status, $shipping_statuses)) {
                ...
                $updated_transaction = Transaction::where('business_id', $business_id)
                    ->where('id', $sell_id)
                    ->where('type', 'sell')
                    ->tap(function ($query) use ($shipping_status, $delivered_to) {
                        $query->update(['shipping_status' => $shipping_status, 'delivered_to' => $delivered_to]);
                    })
                    ->first();

                $client = $this->getClient();

                $this->transactionUtil->activityLog($updated_transaction, 'shipping_edited', $transaction_before, ['from_api' => $client]);
            } else {
                return response()->json([
                    'success' => false,
            ...
            // Step 2: Attach Media to the Model
            $model_id = $sell_id;
            ...
            $model_to_be_attached = $model::where('business_id', $business_id)->findOrFail($model_id);

            // Check if file is uploaded
            if ($request->hasFile('file')) {
                // Only call uploadMedia if the file is valid
                Media::ApiShippingStatus($business_id, $model_to_be_attached, $request, 'file', false, $model_media_type);
            }
