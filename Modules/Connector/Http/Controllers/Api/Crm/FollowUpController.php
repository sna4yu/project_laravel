...
class FollowUpController extends ApiController
{
    ...
    public function index()
    {
        ...
        $query = $this->crmUtil->getFollowUpForGivenDate($user, $start_date, $end_date, $customer_name);
    ...
    public function getLedger(Request $request, $contact_id)
    {
        $user = Auth::user();
        $business_id = $user->business_id;
        ...
        $format = $request->input('format', 'format_1'); // Default to 'format_1' if not provided
        ...
        // Determine line details based on format
        $line_details = ($format == 'format_1');

        // Get ledger details
        $ledger_details = $this->transactionUtil->getLedgerDetails($contact_id, $start_date, $end_date, $format, $location_id, $line_details);

        // Log ledger details for debugging (optional)
        \Log::info('Ledger Details:', [$ledger_details]);

        // Return only ledger_details in JSON response
        return response()->json($ledger_details);
    }
