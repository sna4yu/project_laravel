...
<html lang="en">
    ...
    <div class="centered">
				...
				@if(!empty($receipt_details->letter_head))<img style="width: 60px;margin-bottom: 10px;" src="{{$receipt_details->letter_head}}">@endif
					@if($receipt_details->show_qr_code && !empty($receipt_details->qr_code_text))<img style="width: 60px;margin-bottom: 10px;" src="data:image/png;base64,{{DNS2D::getBarcodePNG($receipt_details->qr_code_text, 'QRCODE')}}">@endif
				</p>
				
				<p class="center width-60 f-right">
					@if(!empty($receipt_details->display_name))<span class="headings">{{$receipt_details->display_name}}</span><br>@endif
			  ...
			  <div class="centered">
					@if(!empty($receipt_details->header_text))<span class="headings">{!! $receipt_details->header_text !!}</span><br/>@endif
					@if(!empty($receipt_details->location_custom_fields)){{ $receipt_details->location_custom_fields }}<br/>@endif
					@if(!empty($receipt_details->tax_info1))<b>{{ $receipt_details->tax_label1 }}</b> {{ $receipt_details->tax_info1 }}@endif
					@if(!empty($receipt_details->tax_info2)){{ $receipt_details->tax_label2 }}</b> {{ $receipt_details->tax_info2 }}@endif	
					@if(!empty($receipt_details->invoice_heading))<br/>
					<span class="sub-headings">{!! $receipt_details->invoice_heading !!}</span>@endif
		</div>

      		
				
		<div class="flex-box">			    
		<p class="text-left">__េមត្តាបង់ប្រាក់ជាមុន__</p> <p class="text-right">@if(!empty($receipt_details->client_id_label))<strong>ID: </strong>{{ $receipt_details->client_id }}</br>@endif</p>
		</div>
						
	
		<div class="flex-box">				
			<p class="width-50 text-left">
				<span class="headings">#: {{$receipt_details->invoice_no}}</br></span>
				<strong>Date: </strong>{{$receipt_details->invoice_date}}</br>
				@if(!empty($receipt_details->due_date_label))<strong>Due: </strong>{{$receipt_details->due_date ?? ''}}</br>@endif
				@if(!empty($receipt_details->commission_agent_label))<strong>Sale: </strong>{{$receipt_details->commission_agent}}</br>@endif
				@if(!empty($receipt_details->customer_rp_label))<strong>{{ $receipt_details->customer_rp_label }}</strong> {{ $receipt_details->customer_total_rp }}</br>@endif
				@if(!empty($receipt_details->additional_notes))Note: <strong>{!! nl2br($receipt_details->additional_notes) !!}</strong>@endif
   ...
   <table style="margin-top: 5px !important" class="border-bottom width-100 table-f-12 mb-10">
                ...
                <tbody>
                	@forelse($receipt_details->lines as $line)
	                    <tr>
	                        ...
	                        <td class="quantity text-right">{{$line['quantity']}} @if($receipt_details->show_base_unit_details && $line['quantity'] && $line['base_unit_multiplier'] !== 1)
                            <br><small>
			...
			@if(!empty($receipt_details->total_quantity_label))
				<div class="flex-box">
					...
					<p class="width-50 text-right">
					@if(!empty($receipt_details->total_items_label)){!! $receipt_details->total_items_label !!}: {{$receipt_details->total_items}}@endif	
					</p>
				</div>
			...
			@if(empty($receipt_details->hide_price))
				...
				@if(!empty($receipt_details->total_due) && !empty($receipt_details->total_due_label))
					<div class="flex-box">
						<p class="left text-right">
						@if(!empty($receipt_details->all_due)){!! $receipt_details->all_bal_label !!}: {{$receipt_details->all_due}}@endif	
						</p>
						<p class="width-50 text-right">
        ...
        <div class="flex-box">				
			<p class="width-50 text-left">
				...
				@if(!empty($receipt_details->due_date_label))<strong>Due: </strong>{{$receipt_details->due_date ?? ''}}</br>@endif
				@if(!empty($receipt_details->commission_agent_label))<strong>Sale: </strong>{{$receipt_details->commission_agent}}</br>@endif
				{!! $receipt_details->all_bal_label !!}  <span class="headings">{{$receipt_details->all_due}}</span></br>
				@if(!empty($receipt_details->additional_notes))Note: <strong>{!! nl2br($receipt_details->additional_notes) !!}</strong></br>@endif
