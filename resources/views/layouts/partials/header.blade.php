...
@if (in_array('pos_sale', $enabled_modules))
    @can('sell.create')
        <a href="{{ action([\App\Http\Controllers\SellPosController::class, 'create']) }}"
            ...
            <svg aria-hidden="true" class="tw-size-5 tw-block" xmlns="http://www.w3.org/2000/svg"
            ...
            <!-- POS Text (Hidden on all screens) -->
            <span class="tw-hidden">POS</span>
        </a>
    @endcan
@endif
...
class="lg:tw-inline-flex tw-transition-all tw-ring-1 tw-ring-white/10 tw-duration-200 tw-bg-@if(!empty(session('business.theme_color'))){{session('business.theme_color')}}@else{{'primary'}}@endif-800 hover:tw-bg-@if(!empty(session('business.theme_color'))){{session('business.theme_color')}}@else{{'primary'}}@endif-700 tw-py-1.5 tw-px-3 tw-rounded-lg tw-items-center tw-justify-center tw-text-sm tw-font-medium tw-text-white hover:tw-text-white tw-font-mono">
    ...
    <!-- Rounded Minus Icon (Visible on all screen sizes) -->
    <svg aria-hidden="true" class="tw-size-5" xmlns="http://www.w3.org/2000/svg"
         viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none"
         stroke-linecap="round" stroke-linejoin="round">
        <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2" fill="red" /> <!-- Circle background in red -->
        <path d="M5 12h14" stroke="currentColor" stroke-width="2" /> <!-- Minus sign -->
    </svg>

    <!-- PAY Text (Visible only on medium and larger screens) -->
    <span class="sm:tw-inline tw-hidden sm:tw-block">PAY</span>
</a>
